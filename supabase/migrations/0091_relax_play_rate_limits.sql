-- Relax play_campaign hourly abuse limits (was too strict for real users / retries).
-- App layer now uses a single burst bucket; these DB caps are the primary guard.
...
begin
  if not check_rate_limit('ip:' || p_ip, 50) then
    return jsonb_build_object('status', 'rate_limited');
  end if;
  if not check_rate_limit('ph:' || p_phone, 15) then
    return jsonb_build_object('status', 'rate_limited');
  end if;
  ...
  if not check_rate_limit('ipcamp:' || v_campaign.id::text || ':' || p_ip, 25) then
    return jsonb_build_object('status', 'rate_limited');
  end if;
  if p_device_id is not null and length(trim(p_device_id)) >= 8 then
    if not check_rate_limit('dev:' || trim(p_device_id), 20) then
      return jsonb_build_object('status', 'rate_limited');
    end if;
    if not check_rate_limit('devcamp:' || v_campaign.id::text || ':' || trim(p_device_id), 12) then
      return jsonb_build_object('status', 'rate_limited');
    end if;
  end if;
