-- Custom joined-date range for campaign customer list.

drop function if exists merchant_list_campaign_customers(uuid, int, timestamptz, uuid, text, text, text, int);
...
language sql stable security definer set search_path = public as $$
  ...
  where c.business_id = p_business_id
    ...
    and (
      case
        when p_joined_from is not null or p_joined_to is not null then
          (p_joined_from is null or c.created_at >= (p_joined_from::timestamp at time zone 'Asia/Kolkata'))
          and (p_joined_to is null or c.created_at < ((p_joined_to + 1)::timestamp at time zone 'Asia/Kolkata'))
        when p_joined_days is not null then
          c.created_at >= now() - make_interval(days => p_joined_days)
        else true
      end
    )
    and (
...
revoke execute on function merchant_list_campaign_customers(uuid, int, timestamptz, uuid, text, text, text, int, date, date)
