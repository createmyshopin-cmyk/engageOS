-- Raise play daily volume cap to 100000 attempts per 24h.
...
declare
  ...
  v_daily_cap constant int := 100000;
begin
  ...
  if not check_rate_limit_window('d24:camp:' || v_campaign.id::text, v_daily_cap, 86400) then
    return jsonb_build_object('status', 'rate_limited');
  end if;
  if not check_rate_limit_window('d24:ipcamp:' || v_campaign.id::text || ':' || p_ip, v_daily_cap, 86400) then
    return jsonb_build_object('status', 'rate_limited');
  end if;
