...
-- WACRM_NOTIFICATIONS
-- ============================================================
CREATE TABLE IF NOT EXISTS wacrm_notifications (
...
CREATE INDEX IF NOT EXISTS idx_wacrm_notifications_user_created
  ON wacrm_notifications(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_wacrm_notifications_user_unread
  ON wacrm_notifications(user_id)
...
ALTER TABLE wacrm_notifications REPLICA IDENTITY FULL;

ALTER TABLE wacrm_notifications ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS notifications_select ON wacrm_notifications;
DROP POLICY IF EXISTS notifications_update ON wacrm_notifications;
CREATE POLICY notifications_select ON wacrm_notifications FOR SELECT
...
CREATE POLICY notifications_update ON wacrm_notifications FOR UPDATE
  USING (auth.uid() = user_id)
...
REVOKE UPDATE ON wacrm_notifications FROM authenticated;
GRANT UPDATE (read_at) ON wacrm_notifications TO authenticated;
...
BEGIN
  ...
  INSERT INTO wacrm_notifications (
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER PUBLICATION supabase_realtime ADD TABLE wacrm_notifications;
  END IF;
END $$;
